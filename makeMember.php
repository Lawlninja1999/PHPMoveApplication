<?php
	...
	if (isset($_POST['submit']))
	{
		...
		if ($_SESSION['username'] == $username)
			{
				$error_message = ''.$username.' Sorry you can not change your user level, please contact another admin and get them to change your user level.';
			}
	...
	else
		{
			...
			YearOfBirth = '".$yearOfBirth."', Country = '".$country."',Password = '".$password."',AccessLevel = 'Member'
			WHERE UserID = ".$_GET['dit_id'];
		...
		if (isset($_GET['dit_id']))
		{
	
			$query = 'SELECT * FROM memberdetails WHERE UserID = '.$_GET['dit_id'];
			$result = $db->query($query);
...
<form name="editMemberForm" method="post">
  <table style="width: 500px; border: 0px;" cellspacing="1" cellpadding="1">
    ...
    <tr style="background-color: #FFFFFF;"> 
      <td> 
		<input type="submit" name="submit" value="Make <?php echo $row['Username'];?> An Member" />
      <td> 
